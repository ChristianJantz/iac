...
variable "aks_region_location" {
    type = string
    description = "The Azure Resource location for the AKS "
}
variable "aks_sku_tier" {
    type = string
    description = "the SKU tier Verion of the AKS. Default Value is Free"
    default = "Free"
...
variable "aks_local_account_disabled" {
    type = bool
    default = null
    description = "(Optional) If 'true' the local account will be disabled. "
}
...
variable "network_profile_network_plugin" {
    type = string
    default = "azure"
    description = "(Required) Specifies the plugin for the Network profile of the AKS"

    validation {
      condition = var.network_profile_network_plugin == "kubenet" || var.network_profile_network_plugin == "azure"
      error_message = "'kubenet' or 'azure' are posible values"
    }
...
variable "network_profile_snet_service_cidr" {
    type = string
    description = "Adressraum für das Kubernetes Cluster "
}
variable "network_profile_snet_dns_service_ip" {
    type = string
    description = "Ip Addresse für die DNS der Nodes im Kubernetes Cluster"
}
variable "load_balancer_sku" {
    type = string
    default = "standard"
    description = "(Optional) Specifies the SKU of the Load Balancer. Posible Values are 'basic' or 'standard'. Default is 'standard'"

    validation {
      condition = contains(["basic", "standard"], var.load_balancer_sku)
      error_message = "Posible values are 'basic' and 'standard'"
    }
}
...
variable "network_profile_load_balancer_sku" {
  ...
  validation {
    condition     = contains(["basic", "standard"], var.network_profile_load_balancer_sku)
...
variable "role_based_access_control_enabled" {
    type = bool
    default = false
    description = "Enable the Azure Active Directory Block"
}
variable "rbac_aad" {
    type = bool
    default = true
...
variable "identity_ids" {
    type = list(string)
    default = null
    description = "(Optional) Specifies a list of User Assigned Managed Identity IDs to be assinged to this Kubernetes Cluster"
}
variable "identity_type" {
    type = string
    default = "SystemAssigned"
    description = "(Optional) The type of identity used for the managed cluster. Default to 'SystemAssigned'"
...
variable "client_id" {
    type = string
    default = ""
    description = "(Optional) The client ID (appID) for Service Principal used for the AKS deployments "
    nullable = false
}
variable "client_secret" {
    type = string
    default = ""
    description = "(Optional) The Client Secret (password) fot Service Principal used for AKS deployment"
    nullable = false
}
...
variable "storage_profile_enabled" {
    type = bool
    default = false
    description = "Enabled storage profile. Default to 'false'"
    nullable = false
}
variable "storage_profile_blob_csi_driver_enabled" {
  type        = bool
  default     = false
